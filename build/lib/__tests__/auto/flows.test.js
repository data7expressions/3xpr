...
describe('flows', () => {
    ...
    test('lab', () => {
        expect(__1.expressions.eval(` list = [1, 2, 3, 4, 5, 6, 7, 8, 9];
					total = 0;
        ...
        expect(__1.expressions.eval(` list = [1, 2, 3, 4, 5, 6];
					b = 1;
					for (a in list) {
						if (b < 10) {
						b = a * b;
        ...
        expect(__1.expressions.eval(` rectangle = {"x":50,"y":50,"width":80,"height":60};
						sleepSecs = 1;
						source=nvl(source,"data/source.jpg");
					`, context)).toStrictEqual('data/source.jpg');
        expect(__1.expressions.eval(` rectangle = {
							"x":50,
							"y":50,
							"width":80,
							"height":60
						...
						source=nvl(source,"data/source.jpg");
					`, context)).toStrictEqual('data/source.jpg');
        expect(__1.expressions.eval(` devices.map(p => 
							concat(p.type,"-",
								if(p.type=="phone") {
									p.imei
								} else {
        ...
        expect(__1.expressions.eval(` while (p=devices.pop()) {
						mac=p.mac;
					}
					mac;
